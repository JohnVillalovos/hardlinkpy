...
# already hardlinked.
def is_already_hardlinked(st1, st2):  # first file's status  # second file's status
...
def eligible_for_hardlink(
    st1, st2, options  # first file's status  # second file's status
):
    ...
    result = (
        ...
        and (  # size is not zero
            (st1[stat.ST_MODE] == st2[stat.ST_MODE]) or (options.contentonly)
        )
        and (  # file mode is the same
            (st1[stat.ST_UID] == st2[stat.ST_UID])
            or (options.contentonly)  # owner user id is the same
        )
        and (  # OR we are comparing content only
            (st1[stat.ST_GID] == st2[stat.ST_GID])
            or (options.contentonly)  # owner group id is the same
        )
        and (  # OR we are comparing content only
            (st1[stat.ST_MTIME] == st2[stat.ST_MTIME])
            or (options.notimestamp)  # modified time is the same
            or (options.contentonly)  # OR date hashing is off
        )
        and (  # OR we are comparing content only
            st1[stat.ST_DEV] == st2[stat.ST_DEV]
        )  # device is the same
            ...
            for (temp_filename, temp_stat_info) in file_hashes[file_hash]:
                ...
                for (temp_filename, temp_stat_info) in file_hashes[file_hash]:
                    if are_files_hardlinkable(
                        work_file_info, (temp_filename, temp_stat_info), options
                    ):
